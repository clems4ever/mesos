...
//      some.
// Then enforcement computes a default value based on share of CPUs.
TEST(MasterResourcesNetworkBandwidthTest, AddDefaultNetworkBandwidth) {
  ...
  ASSERT_SOME(result);
  ASSERT_HAS_NETWORK_BANDWIDTH(task.resources(), NetworkBandwidth(25));
}


// When a task has no network bandwidth reservation and the slave does not
// declare any either,
// Then the task is not updated (we don't add default network bandwidth).
TEST(MasterResourcesNetworkBandwidthTest, SlaveDoesNotDeclareNetworkBandwidth) {
  ...
  ASSERT_SOME(result);
  ASSERT_HAS_NO_NETWORK_BANDWIDTH(task.resources());
}
...
TEST(MasterResourcesNetworkBandwidthTest, SlaveHasNoCpu) {
  ...
  ASSERT_ERROR(result);
  ASSERT_EQ("No CPU advertised by the slave.", result.error());
}
